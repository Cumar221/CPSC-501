...
int test(void)
{
	...
	/* Create an "identity" impulse response. The output should be the same as the input when convolved with this */
	...
	/* Create an"inverse"impulse response. The output should be inverted when convolved with this */
	impulse_response[0] = -1.0;
	impulse_size = 1;
	...
	/* Create a "scaling" impulse response. The output should be 1/2 the amplitude when convolved with this */
	...
	/* Create a "delay" impulse response. The output should be delayed by 2 samples */
	...
	/* Create a"delay and scaling"impulse response. The output should be delayed by 2 samples and be 1/2 the amplitude */
	impulse_response[0] = 0.0;
	impulse_response[1] = 0.0;
	...
	/* Create an "echo of feet". The output will contain the original signal plus a copy delayed by 2 samples and 1/2 the
	* amplitude. The original and copy will overlap starting at the 3rd sample */
	impulse_response[0] = 0.0;
	impulse_response[1] = 0.0;
	impulse_response[2] = 0.5;
	impulse_size = 3;
	...
	/* Creqte an"echo effect"that doesn't overlap. The output will contain the original signal plus a copy delayed by 5 samples and 1/2 the amplitude. */
	impulse_response[0] = 0.0;
	impulse_response[1] = 0.0;
	impulse_response[2] = 0.5;
	...
	/* Interchange the input signal and impulse response. since convolution is commutative, you should get the same output */
	convolve(impulse_response, impulse_size, input_signal, input_size, output_signal, output_size);
	print_vector("Same as above, but with interchanged ht] and ×t]", output_signal, output_size);
...
* 						"Input Side A1gorithm"( see Smith, p. 112-115 ).
*
* 		Parameters:		x[] is the signal to be convolved
* 						N is the number of samples in the vector x[]
* 						h[] is the impulse response, which is convolved with ×[]
